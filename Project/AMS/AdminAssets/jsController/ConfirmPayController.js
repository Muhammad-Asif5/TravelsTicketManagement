...
function viewReport() {
    ...
    $.ajax({
        type: "POST",
        url: "/GenerateInvoice/GetAllConfirmPayableByDate",
        data: {
            Dfrom: Dfrom,
            Dto: Dto,
            AirID: AirID
    ...
    $('#myTable tfoot th').each(function (i) {
        ...
        var title = $(this).text();
        if (title != "Paid2" && title != "Ticket Number" && title != "Net Payable" && title != "") {
...
function myDatatable() {
    ...
    $.ajax({
        type: "POST",
        url: "/GenerateInvoice/GetAllConfirmPayable",
        data: '{}',
...
function OnSuccess(response) {
    ...
    if (response.success == true) {
        ...
        table = $("#myTable").DataTable(
            {
                ...
                scrollY: '50vh',
                "processing": true,
                data: response.PAmount,
                columns: [
                    { "data": "Invoice_Number" },
                    {
                        "data": "Net_Payable", render: function (data, type, row, meta) {

                            if (row.Paid2 == 0 || row.Paid2 == null) {
                                return '<input value="' + row.Net_Payable + '" style="width:150px;" tabindex="' + (meta.row + 1) + '" id="ObtainedMarks' + row.Invoice_Number + '" type="number" name="name" class="form-control" />';
                                //$("#ObtainedMarks" + row.Invoice_Number).val(row.Invoice_Amount);
                            } else {
                                return '<input value="' + row.Paid2 + '" style="width:150px;" tabindex="' + (meta.row + 1) + '" id="ObtainedMarks' + row.Invoice_Number + '" type="number" name="name" class="form-control" />';
                    ...
                    { "data": "Payable_Code" },
                    {
                        "data": "Pay_Status", render: function (data, type, row, meta) {
                            if (row.Pay_Status == "1") {
                ...
                "language": {
                    ...
                    "zeroRecords": "No <b>Confirm Pay Invoice</b> Data Found",
                    //"info": "Total",
        ...
        $('.dataTables_filter input').attr("placeholder", "Search Confirm Pay Invoice");
...
$("#checkAll").change(function () {
    ...
    if (this.checked) {
        $(".RegisterChkbox").each(function () {
            ...
            AmountPayObject.push({ ID, obtainerMarks });
        });
    } else {
        ...
        $(".RegisterChkbox").each(function () {
            this.checked = false;
            var ID = this.id;
            AmountPayObject = AmountPayObject.filter(function (returnableObjects1) {
...
function getUniqueListBy(AmountPayObject, key) {
    return [...new Map(AmountPayObject.map(item => [item[key], item])).values()]
}
...
function saveConfirmInvoice() {
    ...
    TotalInvoiceNoWithArray = [];
    if (AmountPayObject.length > 0) {
        const arr1 = getUniqueListBy(AmountPayObject, 'ID')
...
function UpdateConfirmInvoice(InvNo, Paid) {
    $.ajax({
        type: "POST",
        url: "/GenerateInvoice/SaveConfirmPayable",
        data: {
            InvNo: InvNo,
            Paid2: Paid
        },
